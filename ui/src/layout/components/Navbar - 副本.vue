...
export default {
  ...
  methods: {
    ...
    async logout() {
      ...
      location.href = `http://127.0.0.1:8085/client/index.html#/login`
     ...
     submitPwdForm(formName) {
        this.$refs[formName].validate(async (valid) => {
          if (valid) {
            const res = await fetchChangePwd(this.pwdForm);
//             const res = await fetchChangePwd({
//               newPassword
// : 
// "/mDeJnMXrIJoDASDcjw2cQ==",
// oldPassword
// : 
// "/mDeJnMXrIJoDASDcjw2cQ=="
//             });
