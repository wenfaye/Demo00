 ...
 * 1.获取查询条件，账号，时间，账单类型进行查询
 * 2.根据回签结果回显相符、不相符
 * 3.不相符的可以修改为相符
 * 4.相符的不可以修改为不相符。
 */
function querySignedDetailHF() {

}

// 在prototype里渲染各个页面组件
querySignedDetailHF.prototype = {
	init : function(data) {
		if (!data) {
			//return;
		}
...
//		var cardData = data.ptData;
		...
		/*选择功能*/
		var selectFuc = new PJF.ui.checkbox({
			dom : 'selectFuc',
			singleSelect : true,
			data : [ {
				desc : '已回签账单打印',
		...
		/*核对结果*/
		var checkResult = new PJF.ui.select({
			dom: 'checkResult',
			data: [{desc: '全部', name: '3'},{desc: '相符', name: '1'},{desc: '不符', name: '2'}],
			width : 423,
			onChange: function(){
			}
		});
		...
		var confirmBtn = new PJF.ui.linkButton({
			...
			onClick : function() {
                ...
                //var accType = '-1';
                var accType = data.accType ? data.accType : 1;
                var cstNm = data.cstNm ? data.cstNm : 1;;
				var coinType = moneyType.getValue();// 币种
				var accFlag = checkResult.getValue();// 核对结果
				// 将字符串转为Date格式
...
//				}
                ...
                var jsonData = {
                    ...
                    'DpBkInNo':'',
                	'Sign_St':'1',
                    'Cur_Pcsg_StCd':accFlag,//0 所有未回签1 相符2 不相符3 所有已回签送空默认查全部
				...
				/*var record = [{
					'ACCT_NO':accNo,// 账号
					'ACC_FLAG':accFlag,// 回签结果0 所有未回签1 相符2 不相符3 所有已回签
				...
				data.targetFlag = '1';
				data.btnId = 'btn3';
				data.selectFuc = selectFuc.getValue()[0];
