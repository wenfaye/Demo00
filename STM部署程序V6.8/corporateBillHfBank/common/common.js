...
STM.corporateBill = {
	...
	confirmDeductMoneyByCB: function (pageNo, money, func) {
		...
		if (STM.corporateBill.printRemainCB) {
 ...
 */
	...
	previewHQZD: function (slcData, name, date, code) {
		...
		*/
		...
		var Vchr_Grp = [];
		Vchr_Grp.push({
			"Ovrlsttn_Trck_No": STM.corporateBill.balanceSelfNo,
 ...
 * @async
	...
	queryAndPrintAllData: function (jsonCMST00002, allData, vchIDs, times, i, fileSum, j, sendA07824601Data) {
		...
		console.log(1318, i, i + 1);
		STM.corporateBill.printCBCurTime = i;
		PJF.communication.cpsJsonReq({
			fwServiceId: "simpleTransaction",
			...
			success: function (responseData) {
				console.log(1326, i, i + 1);
				if (responseData.Hd_Grp) {
					if (responseData.Hd_Grp.length) {

						var hdList = responseData.Hd_Grp;

						if (hdList.length > 3 && (hdList.length % 3 != 0)) {
							var lastNum = hdList.length - hdList.length % 3;
							var remainCBList = hdList.slice(lastNum);
							hdList = hdList.slice(0, lastNum);
							...
							console.log(1335, hdList.length, remainVchIDs.length);
				...
				console.log(1365, i + 1);
				if (STM.corporateBill.printCBCurTime + 1 < times) {
					STM.corporateBill.queryAndPrintAllData(jsonCMST00002, allData, vchIDs, times, STM.corporateBill.printCBCurTime + 1, fileSum, j, sendA07824601Data);
				} else {
					if (j < fileSum && '' != vchIDs) {

						var vchIDsLen = vchIDs.substring(0, vchIDs.length - 1).split(",");
						if (vchIDsLen % 3 != 0) {
							console.log(1374, '余数', vchIDsLen);
							/*var slcData = vchIDs.substring(0,vchIDs.length - 1).split(",");
							var lastNum = slcData.length - slcData.length%3;
							var remainCBList = slcData.slice(lastNum);
							slcData = slcData.slice(0,lastNum);
							console.log(1379,slcData.length);*/
						}
					...
					if (STM.corporateBill.priAllCB == true) {
						...
						console.log(1390, allData.length, PJF.util.json2str(allData));
						STM.corporateBill.printAllCBCon(allData);
						// if(times==1){
						// 	STM.corporateBill.printAllCBCon(allData);
						// }else{
						// 	if(i+1 < times){
						// 		STM.corporateBill.printCBArray[i] = allData;
						// 		console.log(1396,times, i+1, fileSum, j);
						// 		STM.corporateBill.queryAndPrintAllData(jsonCMST00002, new Array(), '', times, i+1, fileSum, j,sendA07824601Data);
						// 	}else{
						// 		console.log(1399,STM.corporateBill.printCBArray.length);
						// 	}
						// }
					}
	...
	printAllCB: function (data, prepareData, i, money, pageNo) {
		...
		function continuePrintAllCB(data, prepareData, i, money, pageNo, elec_flag) {
			...
			if (data[i] && data[i].vchIDs && data[i].vchIDs.length) {
				var Hd_Prt_Grp = data[i].vchIDs.split(',').map(function (item, i) {
					return {
						VchID: item
					};
				});

				if (Hd_Prt_Grp.length && Hd_Prt_Grp.length > 30) {
					console.log('warn1432长度超过30:' + Hd_Prt_Grp.length);
					// Hd_Prt_Grp = Hd_Prt_Grp.slice(0,30);
					//Hd_Prt_Grp = Hd_Prt_Grp.slice(0,3);
				}

				sendData.Hd_Prt_Grp = Hd_Prt_Grp;
			...
			if (sendData.Hd_Prt_Grp) {
				console.log(1467, pageNo, sendData.Hd_Prt_Grp.length);
				pageNo += sendData.Hd_Prt_Grp.length;
			}
			console.log(1472, pageNo);
		...
		PJF.util.simpleDownload(locUrl, downUrl, function (d) {
			...
			if (d.success) {//成功
				...
				if (i == (data.length - 1)) {//最后一次下载文件成功
					...
					if (STM.corporateBill.remainCBList && STM.corporateBill.remainCBList.length) {
						// pageNo += STM.corporateBill.remainCBList.split(',').length;
						pageNoStr = pageNo + STM.corporateBill.remainCBList.split(',').length;
					}
					console.log(1603, pageNoStr);
					STM.corporateBill.confirmDeductMoneyByCB(pageNoStr, money, func);
	...
	doPrintFile: function (data, i, printSuccNum, elec_flag) {
		...
		PJF.stm.HDP.startPrintReceipt(startArgs, function (startPrintRes) {
			...
			if ('1' == startPrintRes.status) {//@critical1打印成功
				...
				if (i == (data.length - 1)) {//@critical1最后一次打印成功
					...
					if (STM.corporateBill.priAllCB == true) {
						if (STM.corporateBill.resArr && STM.corporateBill.resArr.length) {
							...
							if (STM.corporateBill.remainCBList && STM.corporateBill.remainCBList.length) {
								var prepareData = [];
								var remainCBList = STM.corporateBill.remainCBList;
								STM.corporateBill.remainCBList = null;
								STM.corporateBill.printRemainCB = true;
								STM.corporateBill.printAllCB([{ vchIDs: remainCBList }], prepareData, 0, 0.00, 0);
							} else {
				...
				console.log('打印出错：' + PJF.util.json2str(startPrintRes) + ',' + data[i]);
				var ppr_Type = data[i] ? data[i].Ppr_TpCd : '03';
				var printProNum = 0;
				if (ppr_Type == "01") { //01:1/3 A4
					printSuccNum += parseInt(startPrintRes.info.printProgressData) || 0;
				...
				if (printSuccNum > 0) {
					...
					if (startPrintRes.errorCode != '5') {
						...
						if ((i + 1) == data.length) {
							...
							//console.log(1903, printSuccNum,PJF.util.json2str(data[i+1]));
							console.log(1943, printSuccNum, STM.corporateBill.ValArray.length);
