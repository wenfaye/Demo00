 ...
 * 3. 预览可预览要打印的回单
 */
function printCBDetailNew() {

}
var previewData = {};
// 在prototype里渲染各个页面组件
printCBDetailNew.prototype = {
	$this : this,
	init : function(data) {
		if (!data) {
			data = {};
			...
			onClick : function() {
				...
				if(0 == data.indexSelect){//回单
					...
					if(slcData.length ==0){
                        ...
                        STM.corporateBill.jsonCMST00003.VchId_Grp = vchIDs.split(',');
		...
		var conditionBtn = new PJF.ui.linkButton({
			...
			name : '自定义打印',
			onClick : function() {
				template.loadPage('printCBCondtionNew', data);
			}
		});
	},
    /**
	...
	createBillList:function(data){
		...
		$this.printCB_grid = new PJF.ui.grid({
			...
			columns :[[
				...
				{title : "摘要",field : "Smy_Dsc",align : 'center',width:180 },
