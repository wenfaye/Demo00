<template>
  <layout ref="layout" :list-title="pageSet.title" :stat-visible="checkStat" >
    ...
    <template #list>
      <div style="height: calc(100% - 35px);margin-bottom: 10px;">
        <el-table
          ref="gridRef"
...
const loadConf = () => {
  var meta = router.currentRoute.value.query.meta;
  list(meta).then((response) => {
    document.title = response.title;
    pageSet.title = response.title;
    pageSet.quick_search = response.quick_search;
    pageSet.actions = response.actions;
    pageSet.grid.columns = response.grid.columns;
    // pageSet.grid.pagination = response.grid.pagination;
    pageSet.data_source = response.data_source;
